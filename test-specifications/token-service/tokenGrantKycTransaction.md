...
| 7       | Grants KYC of a token to an account but signs with an incorrect private key  | tokenId=<CREATED_TOKEN_ID>, accountId=<CREATED_ACCOUNT_ID>, commonTransactionParams.signers=[<INCORRECT_VALID_PRIVATE_KEY>] | The token KYC grant fails with an INVALID_SIGNATURE response code from the network.               | N                 |
...
| 3       | Grants KYC of a token to an account with no account ID | tokenId=<CREATED_TOKEN_ID>, commonTransactionParams.signers=[<CREATED_TOKEN_KYC_KEY>]                                 | The token KYC grant fails with an INVALID_TOKEN_ID response code from the network.    | N                 |
