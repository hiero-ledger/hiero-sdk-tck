...
describe("NodeDeleteTransaction", function () {
  ...
  describe("Node ID", function () {
    it("(#1) Deletes a node with valid node ID", async function () {
      const adminKey = process.env.OPERATOR_ACCOUNT_PRIVATE_KEY as string;
      const { nodeId } = await createTestNode(adminKey, this);
    ...
    it("(#5) Delete a node with incorrect private key", async function () {
      ...
      // Create new operator account, since the treasury account has privileges to delete nodes
