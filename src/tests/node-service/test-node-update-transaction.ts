...
describe("NodeUpdateTransaction", function () {
  ...
  const createTestNode = async (adminKey: string, context: any) => {
    const { accountId, accountKey } = await createTestAccount(context);
    ...
    const ipAddressV4 = toHexString(new Uint8Array([127, 0, 0, 1]));
    ...
    return { nodeId, adminKey, accountId, accountKey, ipAddressV4 };
  };
  ...
  before(async function () {
    ...
    if (!nodeId) {
      const testNodeKey = process.env.OPERATOR_ACCOUNT_PRIVATE_KEY as string;
      const node = await createTestNode(testNodeKey, this);
      nodeId = node.nodeId;
      adminKey = node.adminKey;
      ipAddressV4 = node.ipAddressV4;
    }
  });
  ...
  describe("AdminKey", function () {
    beforeEach(async function () {
      const testNodeKey = await generateEd25519PrivateKey(this);
      const node = await createTestNode(testNodeKey, this);
      nodeId = node.nodeId;
      adminKey = node.adminKey;
      ipAddressV4 = node.ipAddressV4;
