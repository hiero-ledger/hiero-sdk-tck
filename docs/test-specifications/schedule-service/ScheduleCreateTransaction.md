...
| 2       | Creates a scheduled transaction with no scheduled transaction        |                                                    | The schedule creation fails with an `INVALID_TRANSACTION` response code from the network.              | N                 |
