...
| 2       | Sign a schedule with no new valid signature                         | scheduleId=<VALID_SCHEDULE_ID>, commonTransactionParams.signers=[<INVALID_KEY>]         | Fails with `SOME_SIGNATURES_WERE_INVALID`. **Verification**: ScheduleInfo signatories array unchanged.                                                              | N                 |
...
| 5       | Sign a schedule that requires multiple signatures (all signatures)  | scheduleId=<MULTI_SIG_SCHEDULE_ID>, commonTransactionParams.signers=[<SIGNER_KEY_1>, <SIGNER_KEY_2>]    | Transaction succeeds, schedule executed immediately. **Verification**: ScheduleInfo executed_timestamp is set; signatories array contains both expected entries.    | N                 |
...
| 7       | Sign a schedule with insufficient transaction fee                   | scheduleId=<VALID_SCHEDULE_ID>, commonTransactionParams=[maxTransactionFee=<LOW_FEE>]              | Fails with `INSUFFICIENT_TX_FEE`. **Verification**: ScheduleInfo signatories array unchanged.                                                                       | N                 |

---

### **Transaction Execution Scenarios**
| 1       | Given a created schedule with payer account ID that was deleted, sign the schedule with the required keys                            | scheduleId=<VALID_SCHEDULE_ID>, commonTransactionParams.signers=[<SIGNER_KEY>]    | The schedule sign executes successfully and the underlying transaction fails with `PAYER_ACCOUNT_DELETED` by verifying the underlying transaction's receipt.                                                                   | N                 |
| 2       | Given a created schedule with payer account ID that did not sign the creation, sign the schedule with the required keys                            | scheduleId=<VALID_SCHEDULE_ID>, commonTransactionParams.signers=[<SIGNER_KEY>]    | The schedule sign executes successfully and the underlying transaction fails with `INVALID_SIGNATURE` by verifying the underlying transaction's receipt.                                                                   | N                 |
| 3       | Given a created schedule with payer account ID that has 0 balance, sign the schedule with the required keys                            | scheduleId=<VALID_SCHEDULE_ID>, commonTransactionParams.signers=[<SIGNER_KEY>]    | The schedule sign executes successfully and the underlying transaction fails with `INSUFFICIENT_PAYER_BALANCE` by verifying the underlying transaction's receipt.                                                                   | N                 |
...
| 5       | Given a created schedule (crypto transfer) with valid sender balance, sign the schedule with the required keys                            | scheduleId=<VALID_SCHEDULE_ID>, commonTransactionParams.signers=[<SENDER_KEY>]    | The schedule sign executes successfully and the underlying transaction succeeds by verifying the underlying transaction's receipt.                                                                   | N                 |
