...
| 1       | Given a created schedule with payer account ID that was deleted, sign the schedule with the required keys                            | scheduleId=<VALID_SCHEDULE_ID>, commonTransactionParams.signers=[<SIGNER_KEY>]    | The schedule sign executes successfully and the schedule exists by verifying it can be retrieved via ScheduleInfo.                                                                   | Y                 |
...
| 3       | Given a created schedule with payer account ID that has 0 balance, sign the schedule with the required keys                            | scheduleId=<VALID_SCHEDULE_ID>, commonTransactionParams.signers=[<SIGNER_KEY>]    | The schedule sign executes successfully and the schedule exists by verifying it can be retrieved via ScheduleInfo.                                                                   | Y                 |
