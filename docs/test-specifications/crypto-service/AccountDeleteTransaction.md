...
| 8       | Deletes an account with a token balance                           | deleteAccountId=<CREATED_ACCOUNT_ID>, transferAccountId=<OPERATOR_ACCOUNT_ID>, commonTransactionParams.signers=[<PRIVATE_KEY_OF_CREATED_ACCOUNT>] | The account deletion fails with an TRANSACTION_REQUIRES_ZERO_TOKEN_BALANCES response code from the network. | N                 |
